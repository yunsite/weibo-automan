...
CREATE TABLE `users_time_msg` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT 'pk',
  `user_id` bigint(20) unsigned NOT NULL COMMENT '用户的id',
  `weibo_id` varchar(3000) NOT NULL COMMENT '用户微博id',
  ...
  `is_send` char(1) NOT NULL COMMENT '当前消息是否已经发送，y表示已经发送，n表示未发送，e表示发送时有错误，默认值为n',
  `send_result` varchar(256) NULL COMMENT '发送结果，如果发送失败，列出失败原因。',
  `msg_id` bigint(20) unsigned NULL default 0 COMMENT '发送的消息id，与msg表中的id相对应',
  PRIMARY KEY (`id`),
